...
app.post('/api/players/update-points', (req, res) => {
  console.log(' I am in update-points block')
  const { playerId, performance } = req.body;
  console.log(playerId, performance)

  // Validate input
  if (!playerId || !performance) {
    return res.status(400).json({ error: 'Player ID and performance data are required' });
  }

  // Define points system based on performance metrics
  const pointsSystem = {
    runs: 1,           // Example: 1 point per run
    boundaries:5,     // Example: 2 points per boundary
    sixes: 8,          // Example: 3 points per six
    wickets: 25,       // Example: 10 points per wicket
    maidens: 12,        // Example: 5 points per maiden over
    catches: 8,        // Example: 5 points per catch
    stumpings: 12,     // Example: 10 points per stumping
    runOuts: 12       // Example: 10 points per run out
   // Example: 0.5 points per ball faced
  };

  // Calculate points based on the performance data
  const totalPoints =
    (performance.runs * pointsSystem.runs) +
    (performance.boundaries * pointsSystem.boundaries) +
    (performance.sixes * pointsSystem.sixes) +
    (performance.wickets * pointsSystem.wickets) +
    (performance.maidens * pointsSystem.maidens) +
    (performance.catches * pointsSystem.catches) +
    (performance.stumpings * pointsSystem.stumpings) +
    (performance.runOuts * pointsSystem.runOuts)

  // Update player points in the database
  const query = 'UPDATE players SET currentpoints = ? WHERE player_id = ?';
  connection.query(query, [totalPoints, playerId], (err, result) => {
    if (err) {
